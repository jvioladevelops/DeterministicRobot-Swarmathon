 ...
 ;; Justin Viola
 ;; jviola@cnm.edu
 ;; Cental New Mexico Community College
...
to DFS
  ...
  ask robots [
   ...
   ;;Robots should only process the list when they're not dropping off a rock.
...
to process-list
  ...
  ;;Go forward 1 step. robots can move a max of 1 step each tick
  fd 1
...
This is a continuation of the swarmathon project. In the model we show an example of Depth First Search algorithm. The robots move in a deterministic way to optimize their rock finding mission.

## HOW IT WORKS

The user sets the angle that the robots will travel in. The robot moves from outward from the origin until it encounters a boundary, at that point it will turn around. If it passed over a rock on its path, it will head back to the coordinate where the rock was detected and return it to base.

## HOW TO USE IT

Use the sliders to set the rock cluster type and count. The drop down is used to control the layout of the board (clusters, single rocks, large clusters, random). The last sliders is used to control the angle. Setup and DFS buttons run the program.

## THINGS TO NOTICE

Robots travel outward from the origin along set paths.

## THINGS TO TRY

There are nearly limitless combinations to try. Adjust the sliders to control rock count, and angle to see how the robots perform.
